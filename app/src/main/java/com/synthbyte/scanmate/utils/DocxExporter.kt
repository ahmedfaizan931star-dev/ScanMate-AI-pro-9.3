...
object DocxExporter {
    ...
    suspend fun saveXlsxFromText(
        ...
        try {
            ...
            val safeName = FileUtils.sanitizeFileBaseName(
                filename.ifBlank { "ScanMate_Table_${System.currentTimeMillis()}" }
            )
                .removeSuffix(".xlsx")
                .removeSuffix(".XLSX")
            ...
            ZipOutputStream(FileOutputStream(file).buffered()).use { zip ->
                ...
                zip.putXmlEntry("xl/workbook.xml", """
                    ...
                    <workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
                        <sheets>
                            <sheet name="ScanMate OCR" sheetId="1" r:id="rId1"/>
                        </sheets>
                    </workbook>
                """.trimIndent())
    ...
    suspend fun savePptxFromBitmaps(
        context: Context,
        pages: List<Bitmap>,
        filename: String
    ): File? = withContext(Dispatchers.IO) {
        try {
            ...
            val storageDir = FileUtils.appFolder(context, "Exports") ?: return@withContext null
            val safeName = FileUtils.sanitizeFileBaseName(
                filename.ifBlank { "ScanMate_Slides_${System.currentTimeMillis()}" }
            )
                .removeSuffix(".pptx")
                .removeSuffix(".PPTX")
            ...
            ZipOutputStream(FileOutputStream(file).buffered()).use { zip ->
                ...
                validPages.forEachIndexed { index, bitmap ->
                    zip.putXmlEntry(
                        ...
                        slideWithImageXml(index + 1, slideCx, slideCy)
                    )
    ...
    suspend fun saveDocxText(
        ...
        try {
            ...
            val safeName = FileUtils.sanitizeFileBaseName(
                filename.ifBlank { "ScanMate_Document_${System.currentTimeMillis()}" }
            )
                .removeSuffix(".docx")
                .removeSuffix(".DOCX")
            ...
            val cleaned = DocumentIntelligence.cleanOcrText(text)
                .ifBlank { "ScanMate AI Pro document export" }
    ...
    private fun textToTableRows(text: String): List<List<String>> {
        ...
        val rows = cleaned.lines().mapNotNull { line ->
            ...
            val tabCells = trimmed
                .split('\t')
                .map { it.trim() }
                .filter { it.isNotBlank() }

            val spacedCells = trimmed
                .split(Regex("\\s{2,}"))
                .map { it.trim() }
                .filter { it.isNotBlank() }
        ...
        <p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
            <p:cSld>
                <p:spTree>
                    ...
                    <p:pic>
                        <p:nvPicPr>
                            <p:cNvPr id="${index + 1}" name="ScanMate page $index"/>
                            <p:cNvPicPr>
    ...
    private fun officeThemeXml(): String = """
        ...
        <a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="ScanMate">
            <a:themeElements>
                <a:clrScheme name="ScanMate">
                    <a:dk1><a:srgbClr val="000000"/></a:dk1>
                    <a:lt1><a:srgbClr val="FFFFFF"/></a:lt1>
                    <a:dk2><a:srgbClr val="1F1F1F"/></a:dk2>
                    <a:lt2><a:srgbClr val="F5F5F5"/></a:lt2>
                    <a:accent1><a:srgbClr val="0B57D0"/></a:accent1>
                    <a:accent2><a:srgbClr val="2E7D32"/></a:accent2>
                    <a:accent3><a:srgbClr val="EF6C00"/></a:accent3>
                    <a:accent4><a:srgbClr val="6A1B9A"/></a:accent4>
                    <a:accent5><a:srgbClr val="00838F"/></a:accent5>
                    <a:accent6><a:srgbClr val="C62828"/></a:accent6>
                    <a:hlink><a:srgbClr val="0563C1"/></a:hlink>
                    <a:folHlink><a:srgbClr val="954F72"/></a:folHlink>
                </a:clrScheme>
                <a:fontScheme name="ScanMate">
                    <a:majorFont>
